...
```{r}
part_estimates_3 <- as.data.frame(t(get_dr_parts(data, Y ~ A, A ~ X +Z)))
rownames(part_estimates_3) <- c("Outcome Formula", "Exposure Formula", "Outcome Model Estimate", "IPW Estimate", "$\\frac{1}{n}\\sum \\frac{Y_i A_i}{\\hat{e}(\\textrm{covariates}_i)}$" , "$\\frac{1}{n}\\sum \\hat{y}(\\textrm{covariates}_i)$","$\\frac{\\hat{y}(\\textrm{covariates}_i)*A_i}{\\hat{e}(\\textrm{covariates}_i)}$","Difference")
kable(part_estimates_3, escape = FALSE, caption = "It does!", col.names = NULL, digits = tabledigits)
```
